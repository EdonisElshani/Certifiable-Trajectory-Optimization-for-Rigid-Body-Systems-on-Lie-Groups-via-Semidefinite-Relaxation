...
\begin{proposition}[Differential of left translation on a matrix Lie group: $T_{x}\mathbb{L}_{h}$]
Let $G\subset GL(n,\mathbb{R})$ be a matrix Lie group.
For $h\in G$, define $\mathbb{L}_h:G\to G$ by $\mathbb{L}_h(g)=hg$ \cite{Hall2000LieGroups}.
Then for each $x\in G$ the differential
\[
T_x\mathbb{L}_h: T_xG \to T_{hx}G,
\]
of the left translation map $\mathbb{L}_h:G\to G$ is given by
\begin{align}\label{eq:preliminaries_differential_left_translation}
T_x\mathbb{L}_h(v)=hv.
\end{align}
\end{proposition}
\begin{proof}
...
\[
T_x\mathbb{L}_h(v)=\left.\frac{d}{dt}\mathbb{L}_h(\gamma(t))\right|_{t=0}
=\left.\frac{d}{dt}(h\gamma(t))\right|_{t=0}=h\dot{\gamma}(0),
\]
with the left translation map $\mathbb{L}_h(g)=hg$. Since $h$ is constant and $\dot{\gamma}(0)=v$, it follows that
\[
T_x\mathbb{L}_h(v)=h\dot{\gamma}(0)=hv.
\]
\end{proof}

It can be observed that $\mathbb{L}_{h}(g)=hg$ and $T_x\mathbb{L}_{h}(v)=hv$ have the same matrix expression, but acting on different objects. The left translation map $\mathbb{L}_{h}(g)$ is multiplying group elements $g \in G$, while the differential of the left translation map is the pushforward of tangent vectors $v \in T_{x}G \subset \mathbb{R}^{n \times n}$. The differential of a left translation can be applied to the special embedded matrix Lie groups $SO(n)$ and their corresponding lie algebras $\mathfrak{so}(n)$. 

\paragraph{Differential of left translation $T_R\mathbb{L}_H$ on $SO(n)$}

The left translation map in $SO(n)$ is defined as $\mathbb{L}_{H}(R) = HR$ with $H,R \in G$. Applied to eq. \eqref{eq:preliminaries_differential_left_translation} with $v \in T_{R}G$, it follows that
\begin{align*}
    T_{R}\mathbb{L}_{H}(v)= Hv.
\end{align*}
The tangent vector $v \in T_RG$ can be represented by $v = T_{e}\mathbb{L}_{R}(\omega) = R\omega$ using the left translation map of $\omega \in \mathfrak g := \mathfrak{so}(n)$ resulting in
\begin{align}\label{eq:preliminaries_differential_left_SO}
    T_R\mathbb{L}_H(R \omega) = H(R \omega).
\end{align}
