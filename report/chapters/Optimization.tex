...
Section \ref{sec:semidefinite_optimization_relaxation} gives an 
...
Thereby, a nonconvex polynomial optimization problem is relaxed to a convex semidefinite 
program.
...
semidefinite trajectory optimization framework of \cite{Kang_2026Fast,Kang_2025Global}.
...
\begin{equation}\label{eq:rotation_matrices_so2_R_and_F}
\bm{R}_{i,k}=
\begin{bmatrix}
c_{i,k}&-s_{i,k}\\
s_{i,k}&c_{i,k}
...
\end{equation}
where $\bm{C} \in \mathbb{S}^{n}$ and $\bm{b} \in \mathbb{R}^{m}$ \cite{Yang2024Semidefinite}. The linear map \(\mathcal{A}: \mathbb{S}^{n} \to \mathbb{R}^{m}\) is defined as
\begin{equation}
 \mathcal{A}(\bm{X})
 :=
 \begin{bmatrix}
...
\(\gamma_{\kappa}^{*}\leq p^\star\) \cite{Yang2024Semidefinite}.
...
This property is referred to as finite convergence \cite{Nie2014FiniteConvergence,Lasserre2001Global}.
...
\begin{figure}[!ht]
 ...
 \begin{minipage}[t]{0.15\textwidth}
  \centering
  \includegraphics[width=\textwidth]{pics/sparse_relaxation/a_MD.png}
  \caption*{(a)}
 \end{minipage}
 \hspace{0.08\textwidth}
 \begin{minipage}[t]{0.15\textwidth}
  \centering
  \includegraphics[width=\textwidth]{pics/sparse_relaxation/b_MD.png}
  \caption*{(b)}
 \end{minipage}
 \hspace{0.08\textwidth}
 \begin{minipage}[t]{0.15\textwidth}
  \centering
  \includegraphics[width=\textwidth]{pics/sparse_relaxation/c_MD.png}
  \caption*{(c)}
 \end{minipage}
...
\begin{figure}[!ht]
 ...
 \begin{minipage}[t]{0.44\textwidth}
  \centering
  \includegraphics[width=\textwidth]{pics/sparse_relaxation/dense_moment_relaxation.png}
  \caption*{(a) Dense Moment Relaxation}
 \end{minipage}
 \begin{minipage}[t]{0.22\textwidth}
  \centering
  \includegraphics[width=\textwidth]{pics/sparse_relaxation/sparse_moment_relaxation.png}
...
consists of larger moment matrices, compared to the moment matrices constructed using 
...
The following Section applies the discrete motion planning formulation \eqref{eq:general_motion_optimization}
...
\eqref{eq:rotation_matrices_so2_R_and_F}.
...
\bm{A}_{1,k}\bm{\rho}_{1,0}
-h^2g\bm{M}_1\bm{e}_2
-h^2\bar{\bm{\lambda}}_{\max}\left(\bar{\bm{\lambda}}_{0,k}+\bar{\bm{\lambda}}_{12,k}\right)
&=\bm{0}
\end{aligned}
...
&\quad
-h^2g\bm{M}_2\bm{e}_2
+h^2\bar{\bm{\lambda}}_{\max}\bar{\bm{\lambda}}_{12,k}
=\bm{0}
\end{aligned}
...
\bm{\rho}_{1,0}
\times
\bm{R}_{1,k+1}^{\top} \bar{\bm{\lambda}}_{\max} \bar{\bm{\lambda}}_{0,k+1}
\right)\\
&\quad
+
\left(
\bm{\rho}_{1,12}
\times
\bm{R}_{1,k+1}^{\top}\bar{\bm{\lambda}}_{\max}\bar{\bm{\lambda}}_{12,k+1}
\right)
-u_{\max}\bar u_{k+1}
\Bigr]^{\wedge}
&=\bm{0}
...
\bm{\rho}_{2,12}
\times
\bm{R}_{2,k+1}^{\top}\bar{\bm{\lambda}}_{\max}\bar{\bm{\lambda}}_{12,k+1}
\right)
\right]^{\wedge}
&=\bm{0}
\end{aligned}
...
Here, the moment matrix dimensions are 
\begin{equation}
...
\end{equation}
for links $i=1,2$ and with the requirement $\frac{\Delta t_{SDP}}{\Delta t_{sim}} \in \mathbb{N}$.
...
of past rotations for $\Delta t_{\mathrm{SDP}} > t_j$, becasue the time
\(t_j-\Delta t_{\mathrm{SDP}}\) lies before the simulation start.
Therefore, a prehistory is assumed by
\begin{equation}
\bm F_{i,0\mid j}
=
\begin{cases}
\bm R_i(0)^\top\bm R_i(t_j),
& 0\leq t_j<\Delta t_{\mathrm{SDP}},\\[1mm]
\bm R_i(t_j-\Delta t_{\mathrm{SDP}})^\top\bm R_i(t_j),
& t_j\geq\Delta t_{\mathrm{SDP}}.
\end{cases}
...
relative-step rotation error \(e_{F,j}\) are evaluated. The
step-angle error calculated from the last fine simulation step and is measured
in degrees and degrees per second. The Acrobot is declared
stabilized if
\begin{equation}
