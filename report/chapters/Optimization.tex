...
\end{equation}
\cite{Kang_2026Fast} defines the set \(I_k\), which 
denotes the scalar variables of the \(k\)-th
trajectory clique,
\begin{equation}
 \mathbf{z}(I_k)=
 \begin{bmatrix}
...
and stage cost for transition \(k\) is contained inside \(I_k\).
...
After defining the variable cliques \(I_1,\ldots,I_q\), 
\(p(\mathbf{z})=\sum_{i=1}^{q}p_i(\mathbf{z}(I_i))\) and every
...
dimension \(\binom{|I_i|+\kappa}{\kappa}\). Using the chain-like cliques
...
\end{equation}
Here, 
\(\lambda_{1,i}\geq\lambda_{2,i}\geq\cdots\geq0\) are the eigenvalues of
\(\mathbf{M}_{\kappa,i}\). \(\delta_{\max}\) close to 0 indicates an approximately rank-one solution.
...
structure of the 3D-pendulum POP has to be identified. Here, the set $I_k$ of decision variables 
are ordered as
\begin{align}
    \bm{z}(I_k) = [R_{k+1} \quad F_{k+1} \quad R_{k} \quad F_{k} \quad u_{p,k+1} ]^{\top},
\end{align}
...
It has to be noted that the termial cliques $I_0$ and $I_{N-1}$ can be reduced, 
since they only contain the initial or final states.
...
The acrobot model in maximal coordinates contains uses additional 
velocity variables $v_{i,k}\in\mathbb R^2$, which results in a higher number of optimization variables.
Instead, the maximal coordinate model can be reduced to
\begin{align}
    \mathbf{z}=(\bm{R}_{k+1}, R_k, F_{k+1}, F_k, \lambda_{0,k+1}, \lambda_{12,k+1}, u_{k+1}),
\end{align}
while the constraints still remain polynomial. Therefore, the 
...
Therefore, the reduced full-horizon decision vector contains $n_{z}^{reduced}=13N-1$
scalar variables, compared to the original $n_{z}^{full}=17N+3$ variables of the full model.
...
at $k=0$ and $k=1$. 
Moreover, the terminal variables are contained inside $I_{N-1}$,
therefore, no additional terminal clique is required. The size of the first clique is $n_{c,I_1}=25$, 
whereas the interior cliques have size $n_{c,I_k}=21$.
...
do not contain the
multipliers or the input $\bar u_k$. The protected Hybrid SELF construction exploits this
...
\end{align}
\end{subequations}
for $k=3,\ldots,N-1$. Here, $T_k$ covers the coupled translational and
rotational dynamics, whereas
$K_k$ covers the kinematics of both links. Keeping both links together avoids
discarding their physical coupling. However, for this thesis, $I_1$ 
\eqref{eq:acrobot_full_self_initial_clique} and $I_2$ 
\eqref{eq:acrobot_full_self_interior_cliques} is taken as the first two cliques,
...
$351\times351$ at $k=0$ and $253 \times 253$ at $k=1$.
Nevertheless, for each timesteps inside $k=2,\ldots,N-1$ there
...
$n_z=13N-1$ variables. Its order-two moment matrix has the dimension
\begin{equation}
...
this gives
$n_z=779$ and a dense moment matrix of size
...
%\begin{figure}[!ht]
%    \centering
%    \includegraphics[width=\textwidth]{pics/Optimization/Acrobot/MPC_SDP.png}
%    \caption{SDP-based model predictive control loop for the reduced Acrobot.
%    Only the first extracted control is applied before the boundary data are
%    updated from the simulated state and the problem is solved again.}
...
Section~\ref{subsection:discrete_Acrobot}. The resulting rotations and
...
MPC loop terminates. Otherwise, \(j\) is increased by one, the initial data
