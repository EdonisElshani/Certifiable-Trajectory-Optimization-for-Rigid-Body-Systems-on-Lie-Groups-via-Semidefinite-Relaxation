...
Additionally, the exact discrete Lagrangian is defined by
\begin{equation}
...
Let $G$ be a Lie group with Lie algebra $\mathfrak g$, and let $L_d:G\times G\rightarrow\mathbb R$ be smooth \cite{Lee2008}. 
For any $\delta g_k=T_eL_{g_k}(\eta_k)=g_k\eta_k \in T_{g_k}G$ \eqref{eq:discrete_infinitesimal_configuration_variation}, the partial derivative $D_{g_k}L_d(g_k,f_k)\in T_{g_k}^*G$ is characterized by
\begin{equation}
...
Additionally, the directional derivative can be expressed by the covector $T_e^*L_{g_k}$ and the definition of the differential 
of the left translation $g_k \eta_k = T_eL_{g_k}(\eta_k)$ defined in \eqref{eq:preliminaries_differential_left_translation}, which leads to
\begin{align*}
    \left\langle D_{g_k}L_d(g_k,f_k),\delta g_k\right\rangle
    &=
    \left\langle D_{g_k}L_d(g_k,f_k),g_k \eta_k\right\rangle \\
    &=
    \left\langle D_{g_k}L_d(g_k,f_k),T_eL_{g_k}(\eta_k)\right\rangle \\
    &=
    \left\langle T_e^*L_{g_k}D_{g_k}L_d(g_k,f_k),\eta_k\right\rangle.
\end{align*}
\end{proof}
...
For any $\delta f_k=T_eL_{f_k}(X_k)=f_kX_k\in T_{f_k}G$ \eqref{eq:discrete_relative_group_variation}, 
...
Additionally, the directional derivative can be expressed by the covector $T_e^*L_{f_k}$ and the
definition of the differential of the left translation $f_k X_k = T_eL_{f_k}(X_k)$ 
