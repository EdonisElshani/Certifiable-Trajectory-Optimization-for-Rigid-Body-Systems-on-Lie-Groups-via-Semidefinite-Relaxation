...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        \node[box, below right=of lagrangian, xshift=-1.3cm, yshift=-0.3cm] (legendre) {Legendre transform.\\ $\mu=\mathbb{F}L(g,\xi)$};
...
\end{align}
where $T_e^*\mathbb{L}_g:T_g^*G\rightarrow\mathfrak g^*$ denotes the dual of the differential $T_e\mathbb{L}_g$ \cite{Lee2008}. Since \eqref{eq:variation_action_after_integration_by_parts} holds for all $\eta \in \mathfrak{g}$ satisfying $\eta(t_0)=\eta(t_f)=0$, the continuous Euler--Lagrange equations on $G$ are defined as
\begin{subequations}
...
be smooth \cite{Hall2000LieGroups}. For any $\delta\xi\in\mathfrak g$ the partial derivative $D_\xi L(g,\xi)\in\mathfrak g^*$ is characterized by
...
Let $G$ be a Lie group with Lie algebra $\mathfrak g$, and let $L:G\times\mathfrak g\to\mathbb R$
be smooth and $\eta$ an arbitrary curve in $\mathfrak g$ \cite{Hall2000LieGroups}. For any $\delta g=T_e\mathbb{L}_g(\eta)=g\eta\in T_gG$ \eqref{eq:continuous_lie_group_variations}
...
=
\langle T_{e}^{*}\mathbb{L}_{g} D_g L(g,\xi),\eta \rangle
=
...
=
D_g L(g,\xi)(\delta g)$ \cite{Lee2008}. Additionally, the directional derivative can be expressed by the covector $T_e^*\mathbb{L}_g$ and
the definition of the
differential of the left translation $g \eta = T_e\mathbb{L}_g(\eta)$ defined in \eqref{eq:preliminaries_differential_left_translation}, which leads to
\[
\langle D_g L(g,\xi),\delta g\rangle
= \langle D_g L(g,\xi),g \eta \rangle
= \langle D_g L(g,\xi), T_e\mathbb{L}_g(\eta) \rangle
= \langle T_e ^* \mathbb{L}_g D_g L(g,\xi), \eta \rangle.
\]
\end{proof}
...
In contrast to the Lagrangian $L:G\times\mathfrak{g}\to\mathbb{R}$ formulation, the Hamiltonian formulation represents the state on the cotangent bundle $T^*G$ identified with $G \times \mathfrak{g}^*$ by left trivialization. 
Thereby, the state is written as $(g,\mu)$ with the body-fixed momentum $\mu\in\mathfrak g^*$ \cite{Marsden2001}. 
The Lagrangian and Hamiltonian descriptions relate by the Legendre transformation $\mathbb{F}L:G\times\mathfrak g\rightarrow G\times\mathfrak g^*$
\begin{equation}
    \mathbb{F}L:
    (g,\xi)
    \mapsto
...
\end{equation}
Consequently, applying the Legendre transformation\eqref{eq:continuous_generalized_momentum} on the Euler-Lagrange equations \eqref{eq:continuous_euler_lagrange_lie_group} the Hamiltonian equations of motion are derived as
\begin{subequations}
...
Thereby, the Lagrangian flow evolves the system in time while preserving its geometric structure,
as it consits symplecticity and momentum preservation properties \cite{Lee2008,Marsden2001}.
...
$\Phi_h(g)=\Phi(h,g)$. \cite{Lee2008} defines the left trivialization 
$\phi_L:TG \to G\times\mathfrak g$ by
\begin{equation}\label{eq:left_trivialization_noethers_theorem}
    \phi_L(g,\dot{g}) = (g, g^{-1}\dot{g}),
    \qquad
    \phi_L^{-1}(g,g^{-1}\dot{g}) = (g,\dot{g}).
\end{equation}
...
\begin{equation}
    ...
    \left.\frac{\mathrm d}{\mathrm d\epsilon}\right|_{\epsilon=0}
        \phi_L
        \circ T_g\Phi_{\exp_H(\epsilon\zeta)}(g)
        \cdot (\phi_L^{-1}(g,\xi)),
...
This result is applied in Section~\ref{sec:momentum_conservation_3d_pendulum} to identify and verify the 
momentum preserved by the unforced 3D-pendulum. However, \cite{Marsden2001} states that 
the variational integrator does not generally conserve the continous energy of the system exactly.
However, due to the syplectic structure, the energy is bounded and oscillates around the exact energy over a long-time horizon.
...
T(\bm{\omega}) = \int_{B} \lVert \bm{\omega} \hat{\bm{\rho}} \rVert^{2} \, dm(\rho)
...
\begin{equation}\label{eq:non-standard_inertia}
    \bm{J_d} = \int_{B} \hat{\bm{\rho}} \hat{\bm{\rho}}^\top  \, dm(\rho)
...
\indent Moreover, the derivative $D_{\bm{R}}L(\bm{R,\hat{\bm{\omega}}})$, which is in this model only dependent of the potential energy $V(\bm{R})$, is calculated by \eqref{eq:derivative_rotation} to
\begin{align}
...
In order to obtain the Hamilton equations for the unforced 3D-pendulum model, the Legendre transformation $\mathbb{F}L:(SO(3)\times\mathfrak{so}(3)) \to (SO(3) \times \mathfrak{so}(3)^*)$ \eqref{eq:continuous_generalized_momentum} has to be applied, which results in the expression for the momentum
...
\end{equation}
where $\exp_{SO(3)}(\theta \hat{\bm{e}_3})$ expresses a finite rotation about the gravity direction $\bm{e}_3$ by the angle $\theta$ \cite{Lee2008}. By Noether's Theorem \ref{thm:continuous_noether}, the infinitesimally invariance of the 3D-pendulum's Lagrangian is verified, if 
...
\end{equation}
In consequence, using the previously given infinitesimal variation \eqref{eq:continuous_lie_group_variations}, the corespodning relation of $\delta \bm{R} = \zeta \hat{\bm{e}}_3 \bm{R} = \bm{R} \hat{\bm{\eta}}$ is defined, which variation $\hat{\bm{\eta}}$ can be expressed as
