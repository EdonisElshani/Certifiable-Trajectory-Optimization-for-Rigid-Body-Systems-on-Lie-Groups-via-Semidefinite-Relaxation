...
\begin{table}[htbp]
    ...
    The rank metric $\delta_{\max}$ is the maximum second eigenvalue weight
    over all moment matrices. The trajectory quantities are evaluated using
    the ordered extraction, and the reported runtime is the MOSEK optimizer
    time.}
...
\begin{table}[htbp]
    ...
    For each moment block $j$, $\delta_j$ denotes its normalized second
    eigenvalue weight. Thus,
    $\delta_{\max}=\max_j\delta_j$, while
