...
discretizations derived in Section~\ref{sec:properties_discrete_lagrangian_flow} are examined. The analysis focuses 
...
underlying mechanics \cite{Lee2008}. In contrast, variational integrators are 
constructed from a discrete variational principle and are symplectic, momentum 
preserving, and consist robust long-time energy behaviour \cite{Marsden2001}. 
For the evaluation, the evolution of the total energy, the momentum-map behaviour, 
and the preservation of the Lie-group structure of the 3D-pendulum are compared with 
the classical fourth-order Runge-Kutta method and a projected Runge-Kutta method on \(SO(3)\) 
as reference integrators \cite{CELLEDONI2003421, Hairer2000} (see Table~\ref{tab:integrator_comparison}).
...
For the unforced and controlled Acrobot simulations, the initial conditions are chosen as $\bm{F}_i^{\mathrm{init}} = \bm{R}_i^{\mathrm{init}} = \bm{I}$ for both links.
...
Fig.\ref{fig:simulation_results_acrobot_unforced}(b) shows the position error norm $\|\Delta \bm{x}_k\|$ between the LGVI trajectory and the reconstructed RK4 reference. It can be observed that the position error remains small (0.85 m) and oscillatory over the long-time horizon.

The holonomic constraint residuals $\|\bm{\phi}_{0,k}\|_2$ and $\|\bm{\phi}_{12,k}\|_2$ are shown in Fig.\ref{fig:simulation_results_acrobot_unforced}(c). Both the base constraint residual $\|\bm{\phi}_{0,k}\|_2$ and the elbow constraint residual $\|\bm{\phi}_{12,k}\|_2$ remain close to machine precision over the complete simulation ($< 10^{-11}$). This confirms that the constrained LGVI in maximal-coordinates \eqref{eq:acrobot_full_del_shifted} enforces the fixed-base joint and the elbow joint at the position level.
...
The total energy \(E_k\) of the controlled Acrobot is shown in Fig.~\ref{fig:simulation_results_acrobot_forced}(a). In contrast to the unforced case, the applied torque influences mechanical energy, which results in no energy conservation. Under the same sinusoidal input and for the small timestep \(h=0.001\), the LGVI and RK4 energies show nearly identical oscillatory behaviour, which indicates that the two simulations represent the same forced dynamics consistently.
...
nonlinear programming (NLP) solver \cite{Sangli2025}. Therefore, the corresponding 
...
\begin{figure}[!htbp]
    ...
    \begin{minipage}{0.49\textwidth}
        ...
        \caption*{(a) Center-of-mass trajectory $\bm{x}_k=\bm{R}_k\bm{\rho}_c$.}
    \end{minipage}
    \hfill
    \begin{minipage}{0.49\textwidth}
        \centering
...
for a desired rotation of \(\theta_{x,\mathrm{des}}=180^\circ\). The center of mass follows a smooth path
...
\end{equation}
The relative-rotation matrices $\bm{F}_{i,k}$ are smaller than $\bm{R}_{i,k}$ for all $k$.
...
Section~\ref{sec:3d_pendulum_optimization_results}, as this target is 
...
$1.0$ and $1.4$. Consequently, the raw ordered extraction does
not represent a feasible $SO(2)$ trajectory over the complete horizon, and the
rank-one condition does not provide a global optimality certificate. In particular, the 
...
valid swing-up trajectory has been extracted, and the $180^\circ$ target has the highest
suboptimality gap among all target rotations.
...
is non-tight as it may also be a result of a suboptimal NLP upper bound.
...
$N_{\mathrm{SDP}}\Delta t_{\mathrm{SDP}}=3.0\,\mathrm{s}$.
The bounds are
\begin{equation}
    u_{\max}=5\,\mathrm{N\,m},
    \qquad
...
$\varepsilon_F=2^\circ$, respectively. Both conditions must hold for
...
The same configured tolerances $\varepsilon_R=2^\circ$ and
...
\begin{figure}[!t]
    ...
    finer simulation time steps and control updates.}
...
The increase in failure states cannot be explained solely
